...
all: busybox-1.33.1 curl-7.79.1 dropbear-2020.81 fuzzotron-9e86436 loggedfs-0.9 ngrep-2a9603b nmap-7.90 openssh-8.8p1 openssl-1.1.1k parted-3.4 socat-1.8.0.0 strace-6.1 stunnel-5.72 tcpdump-4.99.1 microsocks-1.0.3
...
fuzzotron-9e86436: openssl-1.1.1k pcre-8.45
	$(DOCKER_BUILD)
	$(GRABBY_HANDS) /output/bin/replay /grabby/replay-9e86436
	$(GRABBY_HANDS) /output/bin/fuzzotron /grabby/fuzzotron-9e86436
...
stunnel-5.72: openssl-1.1.1k
	$(DOCKER_BUILD)
	$(GRABBY_HANDS) /output/bin/stunnel /grabby/$@

microsocks-1.0.3: musl-cross-make
	$(DOCKER_BUILD)
	$(GRABBY_HANDS) /output/bin/microsocks /grabby/$@
